...
public:
    ...
    void execute(gemm_test_params& p) {
        ...
        auto info_fused = std::find_if(pi_fused.begin(), pi_fused.end(), [](const primitive_info& p) {
            return p.original_id.find("gemm") != std::string::npos;
        });
...
#define CASE_GEMM_2IN_FP16_4 { { 1, 2, 2, 3 }, { 1, 2, 3, 2 } }, { 1, 2, 3, 3 }, tensor{ 1 }, tensor{ 0 }, data_types::f16, data_types::f16, data_types::f16, format::bfyx, data_types::f32, format::bfyx
...
TEST_P(gemm_2in_quantize_u8, basic) {
    ...
    create_topologies(
        input_layout("input0", get_input_layout(p, 0)),
        input_layout("input1", get_input_layout(p, 1)),
        data("in_lo", get_mem(get_per_channel_layout(p), -3, -2)),
        data("in_hi", get_mem(get_per_channel_layout(p), 2, 3)),
        data("out_lo", get_mem(get_single_element_layout(p), -127)),
        data("out_hi", get_mem(get_single_element_layout(p), 127)),
...
TEST_P(gemm_2in_scale, basic) {
    ...
    create_topologies(
        input_layout("input0", get_input_layout(p, 0)),
        input_layout("input1", get_input_layout(p, 1)),
        data("scale_data", get_mem(get_per_channel_layout(p), 2.2f)),
        gemm("gemm_prim", { input_info("input0"), input_info("input1") }, data_types::f32),
        eltwise("scale", { input_info("gemm_prim"), input_info("scale_data") }, eltwise_mode::prod, data_types::f16),
        border("b",{input_info("scale")}),
        reorder("reorder_bfyx", input_info("b"), p.default_format, data_types::f32)
    );
